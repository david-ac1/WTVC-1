...
-- RLS Policies for notifications (drop existing policies first)
...
BEGIN
  ...
  -- Exit early if project or owner not found
  IF NOT FOUND OR project_record.owner_id IS NULL THEN
    RETURN NEW;
  END IF;
  ...
  IF NOT FOUND OR sender_username IS NULL THEN
    RETURN NEW;
  END IF;
...
BEGIN
  ...
  INTO project_record
  FROM projects p
  LEFT JOIN profiles pr ON p.submitted_by = pr.id
  WHERE p.id = NEW.project_id;
  ...
  IF NEW.parent_id IS NOT NULL THEN
    ...
    -- Create notification for parent comment author (if parent comment and author exist, and not replying to self)
    IF FOUND AND parent_comment_record.author_id IS NOT NULL AND parent_comment_record.author_id != NEW.author_id THEN
  ...
  -- Create notification for project owner (if project owner exists, not commenting on own project, and not already notified via reply)
  IF project_record.owner_id IS NOT NULL AND 
     project_record.owner_id != NEW.author_id AND 
     (NEW.parent_id IS NULL OR parent_comment_record.author_id IS NULL OR parent_comment_record.author_id != project_record.owner_id) THEN
...
BEGIN
  ...
  -- Exit early if comment, author, or project not found
  IF NOT FOUND OR comment_record.author_id IS NULL THEN
    RETURN NEW;
  END IF;
